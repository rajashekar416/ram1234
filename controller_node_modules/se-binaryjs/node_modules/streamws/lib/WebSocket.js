...
function initAsClient(address, options) {
  ...
  if (options.value.customHeaders) {
    Object.keys(options.value.customHeaders).forEach(function(key) {
      requestOptions.headers[key] = options.value.customHeaders[key]
    });
  }
  ...
  (isNodeV4 ? agent : req).once('upgrade', function(res, socket, upgradeHead) {
    ...
    socket['version-lib'] = requestOptions.headers['version-lib'];
    establishConnection.call(self, Receiver, Sender, socket, upgradeHead);

    // Add ping/pong between server and client
    if (self._options.pingInterval != null &&
        self._options.pingInterval > 0) {
      // true => dontFailWhenClosed
      self.ping(undefined, undefined, true);
    }
...
function establishConnection(ReceiverClass, SenderClass, socket, upgradeHead) {
  ...
  socket.setTimeout(this._options.timeout, function() {
    self.close(1002, "Connection lost.");
    cleanupWebsocketResources.call(this, true);
  ...
  if (this.upgradeReq != null) {
    this._socket['version-lib'] = this.upgradeReq.headers['version-lib'];
    upgradeHead['version-lib'] = this.upgradeReq.headers['version-lib'];
  }
