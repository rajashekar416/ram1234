(function() {
  ...
  */
  ...
  flashValidationErrorMessages = function(err, messages, req) {
    var error, path, _ref, _ref1, _results;
    if ((err != null ? err.errors : void 0) != null) {
      _ref = err.errors;
      _results = [];
      for (path in _ref) {
        error = _ref[path];
        if (((_ref1 = messages[error.path]) != null ? _ref1[error.message] : void 0) != null) {
          _results.push(req.flash('error', messages[error.path][error.message]));
        } else {
          logger.error("Missing message resource for validation type: '" + error.message + "'");
