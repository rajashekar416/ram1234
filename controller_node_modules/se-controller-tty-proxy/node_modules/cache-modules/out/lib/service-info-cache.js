(function() {
  var DEFAULT_SETTINGS, SERVICE_KEY_FORMAT, ServiceInfoCache, netUtils, util, _,
    __slice = [].slice;
  ...
  ServiceInfoCache = (function() {
    ...
    ServiceInfoCache.prototype._register = function(serviceName) {
      var ip, key, updateInterval, _update;
    ...
    ServiceInfoCache.prototype._getFromRedis = function(serviceName, done) {
      ...
      return redisClient.hgetall(key, (function(_this) {
        return function(err, nodesHash) {
          var alive, zombies, _ref;
          if (err != null) {
            return done(err);
          } else if (nodesHash == null) {
            return done(null, []);
          } else {
            _ref = _this._splitLiveFromZombies(nodesHash), alive = _ref[0], zombies = _ref[1];
            if (zombies.length > 0) {
              logger.info("cleaning zombie nodes", {
                zombies: zombies
              });
              redisClient.hdel(key, zombies, function(err) {
                if (err != null) {
